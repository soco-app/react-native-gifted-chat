...
const MessageContainer = (props) => {
    ...
    const renderRow = ({ item, index }) => {
        ...
        if (!item._id && item._id !== 0 && !item.idempotency_key) {
            warning('GiftedChat: `_id` and `idempotency_key` is missing for message', JSON.stringify(item));
        }
        if (!item.user) {
            if (!item.system) {
        ...
        if (messages && messages.length > 0 && user) {
            ...
            const messageProps = {
                ...
                // @ts-expect-error
                user,
                key: (_a = item.idempotency_key) !== null && _a !== void 0 ? _a : item._id,
                currentMessage: item,
                previousMessage,
                inverted,
                nextMessage,
    ...
    const keyExtractor = (item) => {
        if (item.idempotency_key) {
            return item.idempotency_key.toString();
        }
        else {
